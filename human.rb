...
class Hangman
  ...
  def initialize(name)
    ...
    @word = File.readlines('word_list.txt').sample.chomp.downcase until @word.length > 4 && @word.length < 13
    @count = 12
    @guess_word = ''
    length = @word.length
    length.times do
  ...
  def playing_game
    ...
    until @count.zero? || @guess_word == @word
      save_game
      load_game
      ask_player
      check_word_include
  ...
  def save_game
    require 'yaml'
    if @player_input == '1'
      @save = { 'name' => @name, 'word' => @word, 'count' => @count, 'guess_word' => @guess_word,
  ...
  def load_game
    ...
    if @player_input == '2'
      ...
      @save = YAML.safe_load(output.read)
      output.close
      @name = @save['name']
      @word = @save['word']
      @count = @save['count']
